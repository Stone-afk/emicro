...
func NewRedisSlidingWindowLimiter(client redis.Cmdable, key string, maxRate int, interval time.Duration) *RedisSlidingWindowLimiter {
...
func (l *RedisSlidingWindowLimiter) limit(ctx context.Context) (bool, error) {
	now := time.Now()
	return l.client.Eval(ctx, luaSlidingWindow, []string{l.key}, l.maxRate, l.interval, now.UnixMilli()).Bool()
}
